...
module tb_conv1_calc;
  ...
  localparam integer PIPELINE_DEPTH = 4;
  ...
  // Connect DUT inputs from the array
  wire [DATA_BITS-1:0] data_out_0  = data_in[0];
  wire [DATA_BITS-1:0] data_out_1  = data_in[1];
  wire [DATA_BITS-1:0] data_out_2  = data_in[2];
  wire [DATA_BITS-1:0] data_out_3  = data_in[3];
  wire [DATA_BITS-1:0] data_out_4  = data_in[4];
  wire [DATA_BITS-1:0] data_out_5  = data_in[5];
  wire [DATA_BITS-1:0] data_out_6  = data_in[6];
  wire [DATA_BITS-1:0] data_out_7  = data_in[7];
  wire [DATA_BITS-1:0] data_out_8  = data_in[8];
  wire [DATA_BITS-1:0] data_out_9  = data_in[9];
  wire [DATA_BITS-1:0] data_out_10 = data_in[10];
  wire [DATA_BITS-1:0] data_out_11 = data_in[11];
  wire [DATA_BITS-1:0] data_out_12 = data_in[12];
  wire [DATA_BITS-1:0] data_out_13 = data_in[13];
  wire [DATA_BITS-1:0] data_out_14 = data_in[14];
  wire [DATA_BITS-1:0] data_out_15 = data_in[15];
  wire [DATA_BITS-1:0] data_out_16 = data_in[16];
  wire [DATA_BITS-1:0] data_out_17 = data_in[17];
  wire [DATA_BITS-1:0] data_out_18 = data_in[18];
  wire [DATA_BITS-1:0] data_out_19 = data_in[19];
  wire [DATA_BITS-1:0] data_out_20 = data_in[20];
  wire [DATA_BITS-1:0] data_out_21 = data_in[21];
  wire [DATA_BITS-1:0] data_out_22 = data_in[22];
  wire [DATA_BITS-1:0] data_out_23 = data_in[23];
  wire [DATA_BITS-1:0] data_out_24 = data_in[24];
  ...
  // Function to compute expected accumulator value (20‑bit signed)
  function automatic signed [19:0] compute_conv;
    ...
    begin
      ...
      for (k = 0; k < 25; k = k + 1) begin
        ...
        // Extend both operands to 20 bits to avoid warnings
  ...
  always @(posedge clk) begin
    if (rst_n && valid_out_calc) begin
      ...
      exp1 = expected_pipe_1[PIPELINE_DEPTH-1][19:8] + ext_bias(ref_bias[0]);
      exp2 = expected_pipe_2[PIPELINE_DEPTH-1][19:8] + ext_bias(ref_bias[1]);
      exp3 = expected_pipe_3[PIPELINE_DEPTH-1][19:8] + ext_bias(ref_bias[2]);

      if (conv_out_1 !== exp1) begin
        $display("ERROR at time %t: conv_out_1 = %d, expected %d", $time, conv_out_1, exp1);
        error_cnt = error_cnt + 1;
      end
      if (conv_out_2 !== exp2) begin
        $display("ERROR at time %t: conv_out_2 = %d, expected %d", $time, conv_out_2, exp2);
        error_cnt = error_cnt + 1;
      end
      if (conv_out_3 !== exp3) begin
        $display("ERROR at time %t: conv_out_3 = %d, expected %d", $time, conv_out_3, exp3);
        error_cnt = error_cnt + 1;
      end
    end
  end
