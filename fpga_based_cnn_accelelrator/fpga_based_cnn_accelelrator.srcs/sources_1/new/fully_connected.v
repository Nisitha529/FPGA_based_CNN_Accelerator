module fully_connected #(
      ...
      state               <= 1'd0;
        ...
        if (valid_out_fc_tmp0 == 1) begin
          ...
          if (buf_idx == INPUT_WIDTH - 1) begin
				...
				end else begin                                          // Computation phase : Gernate an output per cycle
				  out_idx           <= out_idx + 4'b1;                  // Select the next neuron

					if (out_idx == OUTPUT_NUM - 1) begin
					  out_idx         <= 4'd0;                            // Wrap after last output 
					end

					valid_out_fc_tmp0 <= 1;                               // Pulse for each new output
  ...
  // Pipeline multiplier tree

	// Stage 1 : Compute 24 pair-wise products from 48 inputs + bias
  always @(posedge clk)
    begin
			if (~rst_n) begin
				calc_out_tmp0  <= 0;
	...
	// Stage 3: sum the 12 results into 6 intermediate sums
	always @(posedge clk) begin
		if (~rst_n) begin
			calc_out_tmp37 <= 0;
			calc_out_tmp38 <= 0;
	...
	// Stage 4: sum the 6 results into 3 intermediate sums
	always @(posedge clk) begin
		if (~rst_n) begin
			calc_out_tmp43 <= 0;
			calc_out_tmp44 <= 0;
  ...
  // Stage 5: final sum of the three results
	always @(posedge clk) begin
		if (~rst_n) begin
			calc_out_tmp46 <= 0;
		end else begin
  ...
  assign data_out = calc_out[18:7];
	...
	// Valid pipeline: delay the valid pulse by 5 cycles to align with data_out
	always @(posedge clk) begin
		if (~rst_n) begin
			valid_out_fc_tmp1 <= 0;
			valid_out_fc_tmp2 <= 0;
