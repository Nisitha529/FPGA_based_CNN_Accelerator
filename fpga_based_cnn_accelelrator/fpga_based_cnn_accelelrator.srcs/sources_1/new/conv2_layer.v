module conv2_layer (
	...
	wire        valid_out1_buf;   // valid signal from channel 1 buffer

	// Channel 2 buffer outputs
	...
	// -----------------------------------------------------------------
	wire signed [13:0] conv_out_1;        // 14‑bit result before truncation (calc 1)
	wire signed [13:0] conv_out_2;        // 14‑bit result before truncation (calc 2)
	wire signed [13:0] conv_out_3;        // 14‑bit result before truncation (calc 3)

	// Combined valid signals
	wire               valid_out_buf;      // all three buffers have valid data
	wire               valid_out_calc_1;   // calculator 1 output valid
	wire               valid_out_calc_2;   // calculator 2 output valid
	wire               valid_out_calc_3;   // calculator 3 output valid

	// -----------------------------------------------------------------
	// Bias storage and sign extension
	// -----------------------------------------------------------------
	reg  signed [7:0]  bias     [0:CHANNEL_LEN-1];   // 8‑bit signed bias per output channel
	wire signed [11:0] exp_bias [0:CHANNEL_LEN-1];   // sign‑extended to 12 bits

	// -----------------------------------------------------------------
	// Combined valid signals (AND of all three buffer valid signals)
	// -----------------------------------------------------------------
	assign valid_out_buf = valid_out1_buf & valid_out2_buf & valid_out3_buf;

	// Combined output valid (AND of all three calculator valid signals)
	...
	(
			...
			.rst_n          (rst_n),
			.valid_in       (valid_in),
			.data_in        (max_value_1),          // input from channel 1
	...
	(
			...
			.rst_n          (rst_n),
			.valid_in       (valid_in),
			.data_in        (max_value_2),          // input from channel 2
	...
	(
			...
			.rst_n          (rst_n),
			.valid_in       (valid_in),
			.data_in        (max_value_3),          // input from channel 3
	...
	// Uses weights: conv2_weight_11.mem, conv2_weight_12.mem, conv2_weight_13.mem
	// Combines the three input channels into one output.
	// -----------------------------------------------------------------
	conv2_calc
	#(
	...
	(
			.clk            (clk),
			.rst_n          (rst_n),
			.valid_out_buf  (valid_out_buf),          // all buffers have valid data
			...
			.conv_out_calc  (conv_out_1),            // 14‑bit result before bias
			.valid_out_calc (valid_out_calc_1)
	);

	// -----------------------------------------------------------------
	// Convolution calculator for output channel 2
	// Uses weights: conv2_weight_21.mem, conv2_weight_22.mem, conv2_weight_23.mem
	// -----------------------------------------------------------------
	conv2_calc
	#(
	...
	// Uses weights: conv2_weight_31.mem, conv2_weight_32.mem, conv2_weight_33.mem
	// -----------------------------------------------------------------
	conv2_calc
	#(
	...
	// -----------------------------------------------------------------
	// Final output: truncate the 14‑bit accumulator to 12 bits (bits 13:1)
	// and add the bias.
	// conv_out_? is [13:0]; taking [13:1] discards the LSB, effectively dividing by 2.
	// -----------------------------------------------------------------
